...
os.makedirs('results/modeling/knn_selected', exist_ok=True)
...
def preprocess_data(df):
    """
    Preprocess the data for k-NN classification with selected features
...
def train_knn(X, y):
    ...
    # Scale the features (important for distance-based algorithms like k-NN)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)
    
    # Train k-NN model
    # Starting with k=3 as a reasonable default
    clf = KNeighborsClassifier(
        n_neighbors=3,
        weights='distance',  # Use distance weighting to give more weight to closer neighbors
...
def evaluate_model(y_test, y_pred, le):
    ...
    with open('results/modeling/knn_selected/classification_report.txt', 'w') as f:
    ...
    with open('results/modeling/knn_selected/performance_metrics.txt', 'w') as f:
...
def main():
    """
    Main function to run the k-Nearest Neighbors classification model with selected features
    """
    print("\n=== K-NEAREST NEIGHBORS CLASSIFICATION MODEL WITH SELECTED FEATURES ===")
    ...
    # Train the model
    clf, X_test_scaled, y_test, y_pred, cv_scores, scaler = train_knn(X, y)
    ...
    with open('results/modeling/knn_selected/cross_validation_results.txt', 'w') as f:
        f.write("=== CROSS-VALIDATION RESULTS ===\n")
        f.write(f"Cross-validation scores: {cv_scores}\n")
        f.write(f"Mean CV Score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
    
    print("\n=== K-NN MODEL WITH SELECTED FEATURES ANALYSIS COMPLETE ===")
