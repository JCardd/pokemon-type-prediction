...
os.makedirs('results/modeling/random_forest_selected', exist_ok=True)
...
def preprocess_data(df):
    """
    Preprocess the data for random forest classification with selected features
...
def train_random_forest(X, y):
    ...
    Returns:
        tuple: Trained model, cross-validation scores
    """
    # Split the data
    ...
    # Train Random Forest with initial parameters
    ...
    return (clf, X_test_scaled, y_test, y_pred, 
            cv_scores, scaler, feature_importance)

def evaluate_model(y_test, y_pred, le, feature_importance):
    """
    ...
    with open('results/modeling/random_forest_selected/classification_report.txt', 'w') as f:
    ...
    with open('results/modeling/random_forest_selected/performance_metrics.txt', 'w') as f:
        f.write("=== MODEL PERFORMANCE METRICS ===\n")
        for metric, value in metrics.items():
            f.write(f"{metric}: {value}\n")
    
    # Save feature importance
    feature_importance.to_csv('results/modeling/random_forest_selected/feature_importance.csv', index=False)
...
def main():
    """
    Main function to run the random forest classification model with selected features
    """
    print("\n=== RANDOM FOREST CLASSIFICATION MODEL WITH SELECTED FEATURES ===")
    ...
    # Train the model
    (clf, X_test_scaled, y_test, y_pred, 
     cv_scores, scaler, feature_importance) = train_random_forest(X, y)
    
    # Evaluate the model
    metrics = evaluate_model(y_test, y_pred, le, feature_importance)
    ...
    with open('results/modeling/random_forest_selected/cross_validation_results.txt', 'w') as f:
        f.write("=== CROSS-VALIDATION RESULTS ===\n")
        f.write(f"Cross-validation scores: {cv_scores}\n")
        f.write(f"Mean CV Score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
    
    print("\n=== RANDOM FOREST MODEL WITH SELECTED FEATURES ANALYSIS COMPLETE ===")
