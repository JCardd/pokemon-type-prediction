...
os.makedirs('results/modeling/svm_selected', exist_ok=True)
...
def preprocess_data(df):
    """
    Preprocess the data for SVM classification with selected features
...
def train_svm(X, y):
    ...
    # Train SVM with reasonable parameters for a multi-class problem
    print("Training SVM model with selected features. This may take some time...")
    clf = SVC(
...
def evaluate_model(y_test, y_pred, le):
    ...
    with open('results/modeling/svm_selected/classification_report.txt', 'w') as f:
    ...
    with open('results/modeling/svm_selected/performance_metrics.txt', 'w') as f:
...
def main():
    """
    Main function to run the SVM classification model with selected features
    """
    print("\n=== SUPPORT VECTOR MACHINE CLASSIFICATION MODEL WITH SELECTED FEATURES ===")
    ...
    # Train the model
    clf, X_test_scaled, y_test, y_pred, cv_scores, scaler = train_svm(X, y)
    ...
    with open('results/modeling/svm_selected/cross_validation_results.txt', 'w') as f:
        f.write("=== CROSS-VALIDATION RESULTS ===\n")
        f.write(f"Cross-validation scores: {cv_scores}\n")
        f.write(f"Mean CV Score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
    
    print("\n=== SVM MODEL WITH SELECTED FEATURES ANALYSIS COMPLETE ===")
