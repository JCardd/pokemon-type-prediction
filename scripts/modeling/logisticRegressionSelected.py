...
os.makedirs('results/modeling/logistic_regression_selected', exist_ok=True)
...
def preprocess_data(df):
    """
    Preprocess the data for logistic regression with selected features only
...
def train_logistic_regression(X, y):
    ...
    # Train logistic regression with multi-class strategy
...
def evaluate_model(y_test, y_pred, le):
    ...
    with open('results/modeling/logistic_regression_selected/classification_report.txt', 'w') as f:
    ...
    with open('results/modeling/logistic_regression_selected/performance_metrics.txt', 'w') as f:
...
def main():
    """
    Main function to run the baseline logistic regression model with selected features
    """
    ...
    # Train the model
    clf, X_test_scaled, y_test, y_pred, cv_scores, scaler = train_logistic_regression(X, y)
    ...
    with open('results/modeling/logistic_regression_selected/cross_validation_results.txt', 'w') as f:
        f.write("=== CROSS-VALIDATION RESULTS ===\n")
        f.write(f"Cross-validation scores: {cv_scores}\n")
        f.write(f"Mean CV Score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
    
    print("\n=== LOGISTIC REGRESSION MODEL WITH SELECTED FEATURES ANALYSIS COMPLETE ===")
