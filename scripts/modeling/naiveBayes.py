...
os.makedirs('results/modeling/naive_bayes', exist_ok=True)
...
def preprocess_data(df):
    """
    Preprocess the data for Naive Bayes classification
...
def evaluate_model(y_test, y_pred, le):
    ...
    with open('results/modeling/naive_bayes/classification_report.txt', 'w') as f:
    ...
    with open('results/modeling/naive_bayes/performance_metrics.txt', 'w') as f:
...
def main():
    """
    Main function to run the Naive Bayes classification model
    """
    ...
    with open('results/modeling/naive_bayes/cross_validation_results.txt', 'w') as f:
        f.write("=== CROSS-VALIDATION RESULTS ===\n")
        f.write(f"Cross-validation scores: {cv_scores}\n")
        f.write(f"Mean CV Score: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})")
    
    print("\n=== NAIVE BAYES MODEL ANALYSIS COMPLETE ===")
